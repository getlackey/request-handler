...
Obj.prototype.handleOutput = function (opts) {
    ...
    return function (data) {
        var req = self.req,
            res = self.res,
            options = opts || 'json', // default Media Types Supported
            formatOpts = {};
        ...
        if (whiteList && whiteList.length > 0) {
            fields.forEach(function (field) {
                if (whiteList.indexOf(field) === -1) {
                    throw new errors.NotSupportedError('Using ' + field + ' in the select isn\'t supported');
                }
            });
        }
    }
    ...
    if (whiteList) {
        Object.keys(s).forEach(function (name) {
            if (whiteList.indexOf(name) === -1) {
                throw new errors.NotSupportedError('Fields ' + name + ' is not sortable.');
            }
        });
    }
...
Obj.prototype.limit = function (defaultLimit) {
    ...
    if (limit > self.options.limit || defaultLimit > self.options.limit) {
        throw new errors.NotSupportedError('limit (' + limit + ') is over the max (' + self.options.limit + ')');
    }
...
Obj.prototype.skip = function () {
    ...
    if (maxSkip && skip > maxSkip) {
        throw new errors.NotSupportedError('Skip has a maximum limit of ' + maxSkip);
    }
